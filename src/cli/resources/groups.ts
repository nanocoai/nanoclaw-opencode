  ...
  customOperations: {
    ...
    'config update': {
      ...
      description:
        ...
        '--delivery-mode (envelope or tools-only), --timezone (IANA id like "Europe/Lisbon"; "" clears back to the install default; scheduled-task times follow it immediately, message display after restart).',
      handler: async (args) => {
        const id = args.id as string;
        if (!id) throw new Error('--id is required');
        const row = await getContainerConfig(id);
        ...
        if (args['delivery-mode'] !== undefined || args.delivery_mode !== undefined) {
          ...
          if (mode !== 'envelope' && mode !== 'tools-only') {
