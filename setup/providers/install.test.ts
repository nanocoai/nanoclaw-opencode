...
vi.mock('node:child_process', async (original) => {
  ...
  return {
    ...
    execSync: (command: string, options: object) => {
      ...
      return command.startsWith('node -e ') || command === 'rm -f src/opencode-dockerfile.test.ts'
...
describe('OpenCode setup installation and refresh', () => {
  ...
  it.each(['install', 'refresh'] as const)('cleans only the obsolete Dockerfile guard during %s', async (mode) => {
    const directory = root();
    const stale = path.join(directory, 'src/opencode-dockerfile.test.ts');
    const custom = path.join(directory, 'src/custom-dockerfile.test.ts');
    fs.writeFileSync(stale, '// obsolete skill guard\n');
    fs.writeFileSync(custom, '// local customization\n');

    await applyProviderSkill(skill, directory, { mode });

    expect(fs.existsSync(stale)).toBe(false);
    expect(fs.readFileSync(custom, 'utf8')).toBe('// local customization\n');
  });
