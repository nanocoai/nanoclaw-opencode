...
describe('provider throws with active or queued turns', () => {
  ...
  it('preserves a delivered opening answer and notifies every unanswered queued route once', async () => {
    await runFailure(async function* (pushes) {
    ...
    expect(visibleRows().map((row) => [JSON.parse(row.content).text, row.thread_id])).toEqual([
      // The fork settles delivered replies before reporting abandoned work.
      ['Partial A', 'thread-a'],
      [NOTICE, 'thread-b'],
      [NOTICE, 'thread-d'],
    ]);
